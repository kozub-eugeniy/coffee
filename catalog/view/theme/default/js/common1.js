...
;( function( window ) {
	...
	function UISearch( el, options ) {	
		this.el = el;
		this.inputEl = el.querySelector( 'div > input.search-bar-input' );
		this._initEvents();
	}
...
$('#nav > td').addClass('animNav');
}
else{
jQuery('nav').removeClass("sticky");
$('#nav > td').removeClass('animNav');
}
});
...
function slideEffectAjax() {
	...
	$(".top-cart-content").slideToggle();
	return false;
	stopPropagation();	
	});

 $(document).click(function(event) {
    if ($(event.target).closest("ul.top-cart-content").length) return;
    $("ul.top-cart-content").slideUp();
...
$(function(){
	$('.product-view .rating-links a:first-child').click(function(){
		var a = $('.tab_comment').offset();
		$('html, body').animate({
			scrollTop: a.top-50
			}, 400);
	})
		$('.product-view .rating-box, .product-view .rating-links a:last-child').click(function(){
		var a = $('.tab_comment .down').offset();
		$('html, body').animate({
			scrollTop: a.top-50
			}, 400);
		$('.tab_comment #input-name').focus();
	})
